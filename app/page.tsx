...
export default function Home() {
  ...
  async function fetchAllTransactionsForBusiness(
    businessId: string,
    userId: string
  ) {
    // Supabase/PostgREST often applies a default page size; historical imports can
    // fall outside the first page (making year toggles look "non-reactive").
    const pageSize = 1000;
    let from = 0;
    const all: any[] = [];
  ...
  async function fetchTransactionsForBusinessYear(
    businessId: string,
    userId: string,
    year: number
  ) {
    ...
    async function loadTx() {
      ...
      try {
        ...
        try {
          ...
          const txs = await fetchAllTransactionsForBusiness(
            selectedBusinessId,
            userId
          );
          if (!mounted) return;
          ...
          // Initialize the shared selectedPeriod from the latest transaction
          // if it has not been set yet.
          if (!selectedPeriod && txs.length > 0) {
            const latestDate = txs
          ...
          if (!mounted) return;
          setTxError('Failed to load data');
          setTransactions([]);
        }
      } catch {
        if (!mounted) return;
        ...
        if (mounted) setIsTxLoading(false);
      }
    }
  ...
  // Fetch statement transactions by year using `date` range:
  // [YYYY-01-01, (YYYY+1)-01-01) and business_id.
  useEffect(() => {
    let mounted = true;

    async function loadStatementTxs() {
      setStatementsError(null);

      if (!selectedBusinessId || !selectedYearForStatements) {
        setStatementTxs([]);
        return;
      }
      ...
      try {
        ...
        const txs = await fetchTransactionsForBusinessYear(
          selectedBusinessId,
          userId,
        ...
        if (!mounted) return;
        setStatementsError('Failed to load statement data for that year.');
        setStatementTxs([]);
      } finally {
        if (mounted) setIsStatementsLoading(false);
      }
    }

    void loadStatementTxs();
    return () => {
      mounted = false;
    };
  }, [selectedBusinessId, selectedYearForStatements]);

  const statementTransactions = statementTxs;
  ...
  async function loadInsightHistory() {
    ...
    setInsightRuns(mapped);
    setActiveInsightRunId((prev) => prev ?? (mapped[0]?.id ?? null));
  }
  ...
  async function runInsightAnalysis(extraPrompt?: string) {
    ...
    try {
      ...
      if (inserted) {
        setInsightRuns((prevRuns) => [inserted!, ...prevRuns]);
        ...
        setInsightRuns((prevRuns) => [fallback, ...prevRuns]);
  ...
  async function handleDoImport() {
    ...
    try {
      ...
      if (!selectedBusinessId || !uid2) return;
      const txs = await fetchAllTransactionsForBusiness(selectedBusinessId, uid2);
      setTransactions(txs);
    } catch {
      // ignore – the import already happened
    }
  ...
  return (
    <div>
        ...
        <section className="mb-8">
          ...
          {/* Header for the cash chart area – range buttons removed */}
